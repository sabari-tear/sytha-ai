...
import { FaFileAlt, FaPaperPlane, FaRobot, FaUser, FaArrowLeft, FaTrash } from 'react-icons/fa';
...
export default function DocumentChatPage() {
  ...
  useEffect(() => {
    if (params.id) {
      ...
      const storageKey = `document_chat_${userId}_${params.id}`;
    ...
    <div className="rounded-3xl border border-slate-800/60 bg-slate-900/60 shadow-[0_0_60px_rgba(15,23,42,0.8)] backdrop-blur-xl overflow-hidden h-[calc(100vh-9.5rem)] flex flex-col outline-none focus:outline-none focus:ring-0">
      ...
      <div className="flex items-center justify-between border-b border-slate-800/60 px-6 py-3 bg-slate-900/80">
        <div className="flex items-center gap-3 flex-1">
          <button
            onClick={() => router.push('/chat/documents')}
          ...
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-slate-50">{document.name}</h2>
            <p className="text-xs text-slate-400">{document.description || 'Chat about this document'}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-xs text-slate-400 bg-slate-800/50 px-3 py-1.5 rounded-xl">
      ...
      <div className="flex-1 overflow-y-auto chat-scroll p-6 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-slate-50 mb-2">Start Chatting About This Document</h3>
            <p className="text-sm text-slate-400">Ask questions about the content of this document</p>
              ...
              <div className="flex flex-wrap gap-2 justify-center mt-4">
                {[
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-blue-900/30 text-blue-300 rounded-xl text-xs hover:bg-blue-900/50 hover:text-blue-200 ring-1 ring-blue-700/50 transition-all"
                  >
                    {example}
                  </button>
                ))}
            ...
            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              <div className={`flex gap-3 max-w-3xl ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center shadow-lg ${
                  msg.role === 'user' ? 'bg-brand-500' : 'bg-blue-500'
                }`}>
                  {msg.role === 'user' ? (
                ...
                <div className={`px-4 py-3 rounded-2xl shadow-lg ${
                  ...
                  {msg.role === 'user' ? (
                    ...
                    <div className="text-sm prose prose-invert prose-sm max-w-none">
                      <ReactMarkdown
                        components={{
                          h3: ({node, ...props}) => <h3 className="text-blue-400 font-semibold mt-3 mb-2" {...props} />,
                          h4: ({node, ...props}) => <h4 className="text-blue-300 font-medium mt-2 mb-1" {...props} />,
                          ...
                          code: ({node, ...props}) => <code className="bg-slate-900/50 px-1.5 py-0.5 rounded text-blue-300" {...props} />
                        }}
                      >
                        {msg.content}
                      </ReactMarkdown>
      ...
      <form onSubmit={handleSubmit} className="border-t border-slate-800/60 p-3 bg-slate-900/80">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="px-6 py-3 bg-blue-500 text-slate-50 rounded-2xl hover:bg-blue-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-blue-500/40 hover:shadow-blue-400/50"
          >
            <FaPaperPlane />
          </button>
        </div>
